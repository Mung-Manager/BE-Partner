...
class CustomerSelector(AbstractCustomerSelector):
    ...
    def get_customer_list(self, filters: Optional[dict] = None) -> QuerySet[Customer]:
        """이 함수는 필터로 삭제되지 않은 고객 반려동물, 고객 티켓, 티켓을 포함한 고객 리스트를 가져옵니다.
    ...
    def check_is_exists_customer_by_id(self, customer_id: int) -> bool:
        ...
        Returns:
            bool: 고객이 존재하면 True를 반환하며 존재하지 않으면 예외를 발생시킵니다.
        """
        is_exists = Customer.objects.filter(id=customer_id).exists()
    ...
    def check_is_exists_customer_by_pet_kindergarden_id_and_phone_number(self, pet_kindergarden_id: int, phone_number: str) -> bool:
        ...
        Returns:
            bool: 고객이 존재하면 True를 반환하며 존재하지 않으면 예외를 발생시킵니다.
        """
        is_exists = Customer.objects.filter(pet_kindergarden_id=pet_kindergarden_id, phone_number=phone_number).exists()
