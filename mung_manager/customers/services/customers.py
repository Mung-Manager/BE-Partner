...
class CustomerService(AbstractCustomerService):
    ...
    @transaction.atomic
    def create_customers_by_csv(
        self, user, pet_kindergarden_id: int, csv_file: InMemoryUploadedFile
    ) -> List[Customer]:
        """
        ...
        for idx, row in islice(enumerate(reader), 3, None):
            ...
            if phone_number in [
                customer.phone_number for customer in customer_instances
            ]:
                raise ValidationException(
            ...
            pet_instances.extend(
                CustomerPet(name=pet, customer=customer) for pet in pets
            )
        ...
        original_customer_phone_number_instances = (
            self._customer_selector.get_customer_queryset_by_pet_kindergarden_id(
                pet_kindergarden_id=pet_kindergarden_id
        ...
        for customer_instance in customer_instances:
            if (
                ...
                in original_customer_phone_number_instances
            ):
    ...
    @transaction.atomic
    def toggle_customer_is_active(
        self, user, customer_id: int, pet_kindergarden_id: int
    ) -> Customer:
        """
        ...
        customer, has_updated = update_model(
            instance=customer, fields=fields, data=data
        )
    ...
    def update_customer(
        ...
        if pets_to_add:
            ...
            CustomerPet.objects.bulk_create(
                [CustomerPet(name=pet, customer=customer) for pet in pets_to_add]
            )

        if pets_to_delete:
            pets_to_be_deleted = self._customer_pet_selector.get_undeleted_customer_pet_queryset_by_names_and_customer_id(
