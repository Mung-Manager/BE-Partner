...
def create_response(
    ...
    **kwargs: Any,
    ...
    Args:
        data (Optional[dict], optional): 응답 데이터를 담는 딕셔너리 [기본값은 빈 딕셔너리]
        code (Optional[str], optional): 응답 코드를 나타내는 문자열 [기본값은 "request_success"]
        message (Optional[str], optional): 자세한 응답 메시지를 나타내는 문자열 [기본값은 "Request was successful."]
        status_code (status, optional): 응답의 HTTP 상태 코드 [기본값은 HTTP 200 OK]

    Returns:
        Response: 생성된 API 응답을 담는 Django Rest Framework의 Response 객체
    """
