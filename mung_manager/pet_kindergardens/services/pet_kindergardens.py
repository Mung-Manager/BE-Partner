...
from mung_manager.common.selectors import check_object_or_already_exist
...
class PetKindergardenService:
    ...
    def _get_coordinates_by_road_address(
        self, road_address: str
    ) -> Tuple[float, float]:
        """
        ...
        latitude = (
            floor(float(response_data["documents"][0]["road_address"]["y"]) * 10**6)
            / 10**6
        )
        longitude = (
            floor(float(response_data["documents"][0]["road_address"]["x"]) * 10**6)
            / 10**6
        )
