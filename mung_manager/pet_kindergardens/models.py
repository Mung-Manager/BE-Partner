...
class PetKindergarden(TimeStampedModel):
    ...
    visible_phone_number = ArrayField(
        models.CharField(max_length=16), db_comment="노출 전화번호", size=2
    )
    ...
    short_address = ArrayField(
        models.CharField(max_length=128), db_comment="간단 주소", size=10
    )
...
class RawPetKindergarden(models.Model):
    ...
    id = models.AutoField(
        auto_created=True, primary_key=True, serialize=False, db_comment="펫 유치원 로우 아이디"
    )
    ...
    virtual_tel = models.CharField(
        max_length=16, blank=True, db_column="virtualTel", db_comment="가상 전화번호"
    )
    name = models.CharField(max_length=64, db_column="name", db_comment="이름")
    x = models.DecimalField(
        max_digits=9, decimal_places=6, db_column="x", db_comment="경도"
    )
    y = models.DecimalField(
        max_digits=8, decimal_places=6, db_column="y", db_comment="위도"
    )
    business_start_hour = models.TimeField(db_comment="영업 시작 시간")
    business_end_hour = models.TimeField(db_comment="영업 종료 시간")
    address = models.CharField(max_length=128, db_column="address", db_comment="주소")
    road_address = models.CharField(
        max_length=128, db_column="roadAddress", db_comment="도로명 주소"
    )
    abbr_address = models.CharField(
        max_length=128, db_column="abbrAddress", db_comment="지번 주소"
    )
    short_address = ArrayField(
