...
class PetKindergardenCreateAPI(APIAuthMixin, APIView):
    class InputSerializer(BaseSerializer):
        ...
        road_address = serializers.CharField(required=True, max_length=128, label="도로명 주소")
        abbr_address = serializers.CharField(required=True, max_length=128, label="지번 주소")
        ...
        guide_message = serializers.CharField(required=False, allow_blank=True, default="", label="안내 메시지")
...
class PetKindergardenSearchAPI(APIAuthMixin, APIView):
    ...
    class FilterSerializer(BaseSerializer):
        ...
        limit = serializers.IntegerField(default=10, min_value=1, max_value=50, label="조회 개수")
        offset = serializers.IntegerField(default=0, min_value=0, label="조회 시작 위치")

    class OutputSerializer(BaseSerializer):
        profile_thumbnail_url = serializers.URLField(source="thum_url", label="프로필 이미지 URL")
    ...
    def get(self, request: Request) -> Response:
        ...
        return Response(
            data=pagination_raw_pet_kindergardens_data,
