...
class CustomerListAPI(APIAuthMixin, APIView):
    ...
    class FilterSerializer(BaseSerializer):
        ...
        customer_phone_number = serializers.CharField(
            required=False, help_text="고객 전화번호"
        )
        ...
        limit = serializers.IntegerField(
            default=10, min_value=1, max_value=50, help_text="페이지당 조회 개수"
        )
    ...
    def get(self, request: Request, pet_kindergarden_id: int) -> Response:
        ...
        customers = self._customer_selector.get_customer_list(
...
class CustomerDetailAPI(APIAuthMixin, APIView):
    ...
    def get(
        self, request: Request, pet_kindergarden_id: int, customer_id: int
    ) -> Response:
        check_object_or_not_found(
            self._pet_kindergarden_selector.check_is_exists_pet_kindergarden_by_id_and_user(
                pet_kindergarten_id=pet_kindergarden_id,
...
class CustomerUpdateAPI(APIAuthMixin, APIView):
    class InputSerializer(BaseSerializer):
        ...
        phone_number = serializers.CharField(
            required=True, label="고객 전화번호", validators=[PhoneNumberValidator()]
        )
...
class CustomerTicketListAPI(APIAuthMixin, APIView):
    ...
    class OutputSerializer(BaseSerializer):
        ...
        usage_time_count = serializers.IntegerField(
            source="ticket.usage_time_count", label="사용 가능 시간"
        )
        usage_count = serializers.IntegerField(
            source="ticket.usage_count", label="사용 가능 횟수"
        )
        usage_period_in_days_count = serializers.IntegerField(
    ...
    def get(
        ...
        check_object_or_not_found(
            self._customer_selector.check_is_exists_customer_by_id(
                customer_id=customer_id
            ),
            msg=SYSTEM_CODE.message("NOT_FOUND_CUSTOMER"),
            code=SYSTEM_CODE.code("NOT_FOUND_CUSTOMER"),
        ...
        return Response(
            data=pagination_customer_tickets_data, status=status.HTTP_200_OK
        )
...
class CustomerTicketLogListAPI(APIAuthMixin, APIView):
    ...
    class OutputSerializer(BaseSerializer):
        ...
        ticket_type = serializers.CharField(
            source="customer_ticket.ticket.ticket_type", label="티켓 타입"
        )
        usage_time_count = serializers.IntegerField(
            source="customer_ticket.ticket.usage_time_count", label="사용 가능 시간"
        )
        usage_count = serializers.IntegerField(
            source="customer_ticket.ticket.usage_count", label="사용 가능 횟수"
        )
        used_count = serializers.IntegerField(
            source="customer_ticket.used_count", label="사용 횟수"
        )
        unused_count = serializers.SerializerMethodField(label="사용 가능 횟수")
        reserved_at = serializers.DateTimeField(
            source="reservation.reserved_at", label="예약 일시"
        )
        updated_reserved_at = serializers.DateTimeField(
            source="reservation.updated_at", label="예약 수정 일시"
        )
        expired_at = serializers.DateTimeField(
            source="customer_ticket.expired_at", label="만료 일시"
        )
        is_attended = serializers.BooleanField(
            source="reservation.is_attended", label="출석 여부"
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._customer_selector = CustomerContainer.customer_selector()
        self._customer_ticket_reservation_selector = (
            CustomerContainer.customer_ticket_reservation_selector()
        )
    ...
    def get(
        ...
        customer_ticket_reservation = self._customer_ticket_reservation_selector.get_customer_ticket_reservation_list_by_customer_id(
            customer_id=customer_id,
        )
        ...
        return Response(
            data=pagination_customer_ticket_reservation_data, status=status.HTTP_200_OK
        )
...
class CustomerPetDeleteAPI(APIAuthMixin, APIView):
    ...
    def delete(
        self, request: Request, pet_kindergarden_id: int, customer_id: int, pet_id: int
    ) -> Response:
